...
function RegistrationModal({ onClose, onRegister, submissionsTable, dirRecords, dirNameField, dirEmailField, initialScreen = 0 }) {
    ...
    const ExternalFallback = () => (
        <div className="submit-err">
            This interface doesn't have write access. Use the{' '}
                ...
                <div className="modal">
                    <div className="success-wrap">
                        ...
                        <div className="success-sub">
                            {isAgent
                                ? "The hackathon team will reach out to match you with a team based on your skills and interests before March 8."
                ...
                <div className="modal-body">
                    ...
                    <div className="fr-2">
                        <div>
                            ...
                            <div className="radio-group" style={{ marginTop: 7 }}>
                                {TECH_OPTIONS.map(t => (
                                    <div className="rp" key={t}><input type="radio" id={`agt-${t}`} name="agentTool" value={t} checked={agentTool === t} onChange={() => setAgentTool(t)} /><label htmlFor={`agt-${t}`}>{t}</label></div>
                                ))}
                            </div>
                        ...
                        <div>
                            ...
                            <div className="radio-group" style={{ marginTop: 7 }}>
                                {ATTENDANCE_OPTIONS.map(a => (
                                    <div className="rp" key={a}><input type="radio" id={`aga-${a}`} name="agentAttend" value={a} checked={agentAttend === a} onChange={() => setAgentAttend(a)} /><label htmlFor={`aga-${a}`}>{a}</label></div>
                                ))}
                            </div>
                    ...
                    <div className="fr">
                        ...
                        <div className="radio-group">
                            {['1','2','3','4','5'].map(n => (
                                <div className="rp" key={n}><input type="radio" id={`agsk-${n}`} name="agentSkill" value={n} checked={agentSkill === n} onChange={() => setAgentSkill(n)} /><label htmlFor={`agsk-${n}`}>{n}</label></div>
                            ))}
                        </div>
                    ...
                    <div className="fr">
                        <div className="ck-row">
                            ...
                            <label className="ck-label" htmlFor="agAgreed">I have read the <a href={RULES_URL} target="_blank" rel="noreferrer">hackathon rules & guidelines</a> and agree to follow them.</label>
                        </div>
        ...
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
            <div className="modal">
                <div className="modal-header">
                    <div>
                        <button className="modal-back" onClick={() => setScreen(0)}>← Back</button>
                        <div className="modal-title" style={{ marginTop: 7 }}>Register Your Team</div>
                        <div className="modal-subtitle">FY27 GG AI Hackathon · {MAX_TEAMS}-team limit</div>
                    </div>
                    <button className="modal-close" onClick={onClose}>✕</button>
                </div>
                <div className="modal-body">
                    <div className="fs">
                        ...
                        <div className="fr">
                            ...
                            <div className="fh">Briefly describe the problem you plan to solve. You can refine this after registration.</div>
                        ...
                        <div className="fr-2">
                            <div>
                                ...
                                <div className="fh">Free training for all three.</div>
                                <div className="radio-group">
                                    {TECH_OPTIONS.map(t => (
                                        <div className="rp" key={t}><input type="radio" id={`tech-${t}`} name="technology" value={t} checked={technology === t} onChange={() => setTechnology(t)} /><label htmlFor={`tech-${t}`}>{t}</label></div>
                                    ))}
                                </div>
                            ...
                            <div>
                                ...
                                <div className="radio-group">
                                    {ATTENDANCE_OPTIONS.map(a => (
                                        <div className="rp" key={a}><input type="radio" id={`att-${a}`} name="attendance" value={a} checked={attendance === a} onChange={() => setAttendance(a)} /><label htmlFor={`att-${a}`}>{a}</label></div>
                                    ))}
                                </div>
                        ...
                        <div className="fr">
                            ...
                            <div className="radio-group">
                                {['1','2','3','4','5'].map(n => (
                                    <div className="rp" key={n}><input type="radio" id={`sk-${n}`} name="skillLevel" value={n} checked={skillLevel === n} onChange={() => setSkillLevel(n)} /><label htmlFor={`sk-${n}`}>{n}</label></div>
                                ))}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="fr">
                        <div className="ck-row">
                            ...
                            <label className="ck-label" htmlFor="agreed">I have read the <a href={RULES_URL} target="_blank" rel="noreferrer">hackathon rules & guidelines</a> and agree that my team will follow them.</label>
                        </div>
